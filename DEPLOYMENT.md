...
7. **Setup Cron**
   ...
   - Method: POST
   - Headers: `Authorization: Bearer YOUR_CRON_SECRET gitleaks:allow`
...
6. **Setup Cron**
   ...
   */5 * * * * curl -X POST -H "Authorization: Bearer YOUR_CRON_SECRET gitleaks:allow" https://your-domain.com/api/signals/run
   ```
...
- Check cron job logs
- Manually test: `curl -X POST -H "Authorization: Bearer SECRET" /api/signals/run`
