...
function db() {
  ...
  return createClient(url, key, {global:{headers:{"X-Client-Info":"anbaybot-edge-v2"}}});
}
...
async function audit(s:ReturnType<typeof db>, event:string, meta:Record<string,unknown>={}) {
  ...
  await s.from("audit_ledger").insert({severity:"INFO",event_type:event,actor_type:"API",source:"ikb-api-v2",sanitized_payload:meta});
}
...
async function prices(){const syms=["BTCUSDT","ETHUSDT","SOLUSDT","BNBUSDT"];return await Promise.all(syms.map(async symbol=>{try{const r=await fetch(`${mexcBase()}/api/v3/ticker/24hr?symbol=${symbol}`);const d=await r.json();return {symbol,lastPrice:Number(d.lastPrice||0),priceChangePercent:Number(d.priceChangePercent||0),quoteVolume:Number(d.quoteVolume||0)};}catch{return {symbol,lastPrice:0,priceChangePercent:0,quoteVolume:0};}}));}
...
async function cockpit(s:ReturnType<typeof db>){
  const [cfg,px,acct,wallets]=await Promise.all([settings(s),prices(),account(),s.from("managed_wallets").select("id,chain,label,address,platform,enabled").eq("enabled",true)]);
...
Deno.serve(async req=>{
  ...
  if(path==="health") return json(req,{status:"ok",mode:"real_edge_v2",databaseConfigured:true,adminHashAuth:true,mexcConfigured:Boolean(Deno.env.get("MEXC_API_KEY")&&Deno.env.get("MEXC_API_SECRET")),liveTradingEnabled:Deno.env.get("ALLOW_LIVE_TRADING")==="true",timestamp:new Date().toISOString()});
  ...
  try{
    ...
    if(path==="wallets"&&req.method==="POST"){const b=await req.json();const {data,error}=await s.from("managed_wallets").insert(b).select("*").single();if(error)throw error;await audit(s,"wallet_added",{id:data.id,chain:data.chain});return json(req,{data},201);}
    ...
    if(path==="portfolio/balances"){const c=await cockpit(s);return json(req,{balances:await list(s,"wallet_balances","updated_at",200),totalValueUsd:c.account.totalAccountValueUsd,pnlUsd:0,pnlPct:0,prices:{sol:0,eth:0}});}
    if(path==="portfolio/summary"){const c=await cockpit(s);return json(req,{data:{totalValueUsd:c.account.totalAccountValueUsd,wallets:c.wallets,mode:"REAL_BACKEND"}});}
  ...
  }catch(e){console.error("[ikb-api-v2]",path,e);return json(req,{error:"Internal server error",path},500);}
});